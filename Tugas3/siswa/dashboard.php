...
<div class="container mt-4">
    ...
    <form method="POST" class="row gy-2 gx-3 align-items-center mb-4">
        ...
        <div class="col-auto">
            <a href="dashboard.php?reset=1" class="btn btn-danger">Reset NIS</a>
        </div>
    </form>
        ...
        <nav aria-label="Pagination NIS">
            <ul class="pagination">
                <li class="page-item <?= ($nisPage <= 1 ? 'disabled' : '') ?>">
                    <a class="page-link" href="dashboard.php?nis_page=<?= max(1, $nisPage - 1) ?>">« Sebelumnya</a>
                </li>
                ...
                <li class="page-item <?= ($nisPage >= $nisPages ? 'disabled' : '') ?>">
                    <a class="page-link" href="dashboard.php?nis_page=<?= min($nisPages, $nisPage + 1) ?>">Berikutnya »</a>
                </li>
            </ul>
        </nav>
    <?php } ?>
        ...
        <table class="table table-bordered">
            ...
            <?php while ($h = mysqli_fetch_assoc($history)) { ?>
                <tr>
                    ...
                    <td><?= $h['ket'] ?></td>
                    <td><?= $h['status'] ?></td>
                    <td><a href="status.php?id=<?= $h['id_pelaporan'] ?>" class="btn btn-info btn-sm">Detail</a></td>
                </tr>
            <?php } ?>
        </table>
    <?php } ?>
