...
<nav class="navbar navbar-dark bg-dark">
    <div class="container">
        <a class="navbar-brand" href="siswa_dashboard.php">Siswa Panel</a>
        <div>
            <a href="siswa_form.php" class="btn btn-primary">Kirim Aspirasi</a>
            <a href="siswa_dashboard.php" class="btn btn-light ms-2">Dashboard</a>
        </div>
    </div>
</nav>
        ...
        <nav aria-label="Pagination NIS">
            <ul class="pagination">
                <li class="page-item <?= ($nisPage <= 1 ? 'disabled' : '') ?>">
                    <a class="page-link" href="siswa_histori.php?nis_page=<?= max(1, $nisPage - 1) ?>">« Sebelumnya</a>
                </li>
                ...
                <li class="page-item <?= ($nisPage >= $nisPages ? 'disabled' : '') ?>">
                    <a class="page-link" href="siswa_histori.php?nis_page=<?= min($nisPages, $nisPage + 1) ?>">Berikutnya »</a>
                </li>
            </ul>
        </nav>
    <?php } ?>
    ...
    <?php if (isset($query)) { ?>
        <table class="table table-bordered mt-4">
            ...
            <tr>
                ...
                <td><?= $d['status'] ?></td>
                <td>
                    <a href="siswa_status.php?id=<?= $d['id_pelaporan'] ?>" class="btn btn-info btn-sm">Detail</a>
                </td>
            </tr>
            <?php } ?>
